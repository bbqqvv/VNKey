...
fn reproduce_modifier_cancellation_bug() {
    ...
    assert_eq!(
        engine.feed_str("aaa"),
        "aa",
        "aaa should cancel and produce aa, not âa"
    );
}
...
fn reproduce_ee_cancellation_bug() {
    ...
    assert_eq!(
        engine.feed_str("eee"),
        "ee",
        "eee should cancel and produce ee, not êe"
    );
}
...
fn reproduce_oo_cancellation_bug() {
    ...
    assert_eq!(
        engine.feed_str("ooo"),
        "oo",
        "ooo should cancel and produce oo, not ôo"
    );
}
...
fn reproduce_dd_cancellation_bug() {
    ...
    assert_eq!(
        engine.feed_str("ddd"),
        "dd",
        "ddd should cancel and produce dd, not đd"
    );
}
