...
pub const VOWEL_CLUSTERS: &[&str] = &[
    ...
    "oa", "oă", "oe", "oi", "ôi", "ơi", "ua", "uâ", "uê", "ui", "uô", "uơ", "ưa", "ươ", "uy", "uya", "uyê",
];
...
impl Syllable {
    pub fn is_valid(&self) -> bool {
        if self.vowel.is_empty() { return false; }
        ...
        match self.onset.as_str() {
            ...
            "g" | "ng" | "c" => {
                ...
                if is_front && self.onset != "gi" { return false; }
            },
            "q" => {
                // Q must be followed by U
                if !self.vowel.starts_with('u') { return false; }
            },
            "p" => {
                // P only starts the onset in few loan words
                if !matches!(self.vowel.as_str(), "i" | "o" | "ô" | "u") { return false; }
            }
            _ => {}
        }
        ...
        if self.vowel.starts_with('y') && !self.onset.is_empty() {
             if !matches!(self.onset.as_str(), "h" | "k" | "l" | "m" | "n" | "q" | "t" | "v") { return false; }
        }

        // - Stop codas (c, ch, p, t) only take Sắc/Nặng
        if matches!(self.coda.as_str(), "c" | "ch" | "p" | "t") {
            if self.tone != 1 && self.tone != 5 { return false; }
        }
        ...
        if matches!(self.coda.as_str(), "ch" | "nh") {
            ...
            if !matches!(last_v, 'a' | 'e' | 'ê' | 'i') { return false; }
        }
